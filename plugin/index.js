...
function warnMissingApiKey(log, context) {
  const heading = "[memos-local] Missing MEMOS_API_KEY (Token auth)";
...
export default {
  ...
  name: "MemOS Local OpenClaw Plugin",
  ...
  register(api) {
    ...
    if (!cfg.envFileStatus?.found) {
      ...
      log.warn?.(`[memos-local] No .env found in ${searchPaths}; falling back to process env or plugin config.`);
    }

    if (cfg.conversationSuffixMode === "counter" && cfg.resetOnNew) {
      if (api.config?.hooks?.internal?.enabled !== true) {
        log.warn?.("[memos-local] command:new hook requires hooks.internal.enabled = true");
      }
      api.registerHook(
        ["command:new"],
        (event) => {
        ...
        {
          name: "memos-local-conversation-new",
    ...
    api.on("before_agent_start", async (event, ctx) => {
      ...
      try {
        ...
        log.warn?.(`[memos-local] recall failed: ${String(err)}`);
      }
    });
    ...
    api.on("agent_end", async (event, ctx) => {
      ...
      try {
        ...
        log.warn?.(`[memos-local] add failed: ${String(err)}`);
      }
    });
  },
};
