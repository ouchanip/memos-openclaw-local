...
export async function callApi({ baseUrl, apiKey, timeoutMs = 5000, retries = 1 }, path, body) {
  ...
  // Auth is optional for self-hosted MemOS
  if (apiKey) {
...
/**
 * Convert self-hosted MemOS /product/search response to cloud-compatible format.
 * Cloud format: { data: { memory_detail_list, preference_detail_list, tool_memory_detail_list } }
 * Self-hosted:  { data: { text_mem: [{cube_id, memories: [...]}], pref_mem, tool_mem } }
 */
function adaptSearchResponse(raw) {
  const data = raw?.data;
  if (!data) return raw;

  // Already in cloud format
  if (data.memory_detail_list) return raw;

  const memoryDetailList = [];
  for (const cube of (data.text_mem || [])) {
    for (const mem of (cube.memories || [])) {
      memoryDetailList.push({
        memory_key: mem.metadata?.key || "",
        memory_value: mem.memory || "",
        create_time: mem.metadata?.created_at || "",
        memory_id: mem.id,
        confidence: mem.metadata?.confidence,
        relativity: mem.metadata?.relativity,
      });
    }
  }

  const preferenceDetailList = [];
  for (const cube of (data.pref_mem || [])) {
    for (const mem of (cube.memories || [])) {
      preferenceDetailList.push({
        preference: mem.memory || mem.metadata?.key || "",
        preference_type: mem.metadata?.type || "explicit",
        create_time: mem.metadata?.created_at || "",
      });
    }
  }

  const toolMemoryDetailList = [];
  for (const cube of (data.tool_mem || [])) {
    for (const mem of (cube.memories || [])) {
      toolMemoryDetailList.push({
        tool_value: mem.memory || "",
        create_time: mem.metadata?.created_at || "",
      });
    }
  }
  ...
  return {
    ...
    data: {
      ...
      preference_note: data.pref_note || data.pref_string || "",
      tool_memory_detail_list: toolMemoryDetailList,
    },
  };
}

export async function searchMemory(cfg, payload) {
  // Self-hosted: /product/search, Cloud: /search/memory
  const searchPayload = {
    user_id: payload.user_id,
    query: payload.query,
  };
  const raw = await callApi(cfg, "/product/search", searchPayload);
  return adaptSearchResponse(raw);
}

export async function addMessage(cfg, payload) {
  // Self-hosted: /product/add, Cloud: /add/message
  const addPayload = {
    user_id: payload.user_id,
    messages: payload.messages,
  };
  if (payload.conversation_id) addPayload.conversation_id = payload.conversation_id;
  if (payload.tags) addPayload.tags = payload.tags;
  return callApi(cfg, "/product/add", addPayload);
}
...
function buildPromptFromData(data, options = {}) {
  ...
  const lines = [
    ...
    "* **Special Note**: Content tagged with '[assistant観点]' or '[モデル総結]' represents **past AI inference**, **not** direct user statements.",
    ...
    "* If a memory has tags like '[assistant観点]' or '[モデル総結]', treat it as a **hypothesis**, not a user-grounded fact.",
    ...
    "* If the memory describes a **third party**, never attribute it to the user.",
    "",
